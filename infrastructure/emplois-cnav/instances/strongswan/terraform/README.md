...
┌───────────────────────────────────────────────────────────────────┐
│ Tunnel IPsec                                                      │
│ Le paquet matche les selectors : local_ts → remote_ts             │
...
La PSK doit être définie manuellement dans l'instance, dans la section `secrets` de
`/etc/swanctl/conf.d/vpn-bcrs.conf`, puis rechargée avec `swanctl --load-creds`, faute de pouvoir le faire de
manière sécurisée depuis Terraform / Scaleway.  
...
- État du tunnel : `swanctl --list-sas` (IKE_SA `ESTABLISHED`, CHILD_SA `INSTALLED`, compteurs `in` / `out`)
- Journaux : `journalctl -u strongswan -f`
- Recharger la configuration après modification : `swanctl --load-all`

L'accès SSH à l'instance et au bastion de la public gateway est limité aux clés SSH IAM du projet `emplois-cnav`,
gérées dans le module `emplois-cnav/iam`.
