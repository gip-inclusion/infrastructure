locals {
  ...
  instance_user_data = {
    ...
    write_files = [
      ...
      # Written as a conf.d drop-in, picked up by the `include conf.d/*.conf` of the stock swanctl.conf, so the
      # package conffile is never overwritten. The PSK is left as a placeholder and must be set manually on the VM
      # post-deploy (this keeps the secret out of Terraform state), then loaded with `swanctl --load-creds`.
      {
        path = "/etc/swanctl/conf.d/vpn-bcrs.conf"
        content = base64encode(
          templatefile("${path.module}/templates/vpn-bcrs.conf.tpl", {
      ...
      # Overrides for the swanctl section shipped by the package, see the template.
      {
        path        = "/etc/strongswan.d/swanctl.overrides.conf"
        content     = base64encode(file("${path.module}/templates/swanctl.overrides.conf"))
        encoding    = "b64"
        permissions = "0600"
      },
    ...
    runcmd = [
      ...
      # from local_ts. We use DNAT+SNAT:
      ...
      # SNAT: Change source IP so traffic matches IPsec tunnel selectors (local_ts)
